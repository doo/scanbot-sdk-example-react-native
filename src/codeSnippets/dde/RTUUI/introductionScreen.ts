...
async function startScanning() {
  try {
    ...
    introductionConfiguration.image = new DocumentDataIntroCustomImage({
      uri: 'imageUri',
    });
    /** Configure the text. **/
    configuration.introScreen.explanation.color = '#000000';
    configuration.introScreen.explanation.text =
      'Quickly and securely scan your document details by holding your device over the document, ' +
      'so that the camera aligns with all the information on the document.' +
      '\\nThe scanner will guide you to the optimal scanning position.' +
      'Once the scan is complete, your document details will automatically be extracted and processed.';
